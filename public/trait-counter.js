...
(async () => {
  let results = {}, maxPerTrait = {}, totalNFTs = 0;
  let page = 0, total = [];
  ...
  for (const nft of total) {
    ...
    for (const attr of attributes) {
      ...
      maxPerTrait[type] = Math.max(maxPerTrait[type] || 0, results[type][value]);
    }
  }

  fs.writeFileSync(output, JSON.stringify({ counts: results, maxPerTrait: maxPerTrait, totalNFTs: totalNFTs }, null, 2));
